...
local function findBlock()
    ...
    local block = nil
    local playerPosition = game.Players.LocalPlayer.Character.HumanoidRootPart.Position
    local patternFarm = getPatternFarm()
    ...
    for _, v in pairs(game:GetService("Workspace").__THINGS.__INSTANCE_CONTAINER.Active.AdvancedDigsite.Important.ActiveBlocks:GetChildren()) do
        if v.BrickColor == "Really black" then continue end
        if v:IsA("BasePart") then
            local coord = v:GetAttribute("Coord")
...
local function getBlocksByLevel(level)
    local blocks = {}
    local activeBlocks = game:GetService("Workspace").__THINGS.__INSTANCE_CONTAINER.Active.AdvancedDigsite.Important.ActiveBlocks
    for _, v in pairs(activeBlocks:GetChildren()) do
        if v:IsA("BasePart") then
            local coord = v:GetAttribute("Coord")
            if coord and coord.Y == level then
...
function startDigging()
    ...
    local lastChestFoundAt = os.clock()
    local noBlocksCount = 0

    while task.wait(0.1) and config.autoDig do
        if not game:GetService("Workspace").__THINGS.__INSTANCE_CONTAINER.Active:FindFirstChild("AdvancedDigsite") then
